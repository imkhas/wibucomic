...
export async function searchManga(query: string, limit = 20, offset = 0) {
  ...
  params.append('contentRating[]', 'erotica'); // Added erotica
...
export async function getMangaFeed(mangaId: string, language = 'en', limit = 100, offset = 0) {
  const params = new URLSearchParams({
    limit: limit.toString(),
    offset: offset.toString(),
    'translatedLanguage[]': language, // Use dynamic language
    'order[chapter]': 'asc',
    'order[volume]': 'asc',
  });
...
export function extractAuthorName(relationships: MangaDexManga['relationships']): string | null {
  ...
  return author?.attributes?.name || author?.id || null;
}
